...
],weekendFood:173.33,general:100,transactions:[],carry:0,carryHistory:[],stashHistory:[],monthlyActions:{},incomeHistory:[],startDate:"",payDay:1,paychecksPerYear:12,hiddenBudgetSections:{},skippedPaychecks:{},autoBackup:true,backupIntervalMinutes:15,dark:false,theme:"iconic",openMonths:{},customSections:[]};
let state;
try{
  const saved=JSON.parse(localStorage.getItem("moneywise")||"null");
  if(saved && /^0\.([1-9][0-9]*)$/.test(String(saved.version)) && Number(String(saved.version).split(".")[1])>=11 && Number(String(saved.version).split(".")[1])<=26){
...
function expenses(k=monthKey()){return txs(k).filter(x=>x.type!=="income"&&!x.budgetSection)}
...
function netCashFlow(){return state.transactions.reduce((a,x)=>a+(x.type==="income"?1:-1)*(Number(x.amount)||0),0)}
...
function processPaycheckBudgetDeductions(month=monthKey()){
  ...
  ["recurring","gifts",...(state.customSections||[]).map((_,i)=>`custom-${i}`)].forEach(key=>{
    if(state.budgetDeduct?.[key]?.onPaycheck)changed=deductBudgetSection(key,month,false)||changed;
  });
  if(changed)save();
}
...
function remaining(){return available()-spent()} function availableThisMonth(){return Math.max(0,(Number(state.flexible)||0)-spent(monthKey()))}
...
function cashflow(){ return `<div class="top"><h1>Cash Flow</h1><div class="muted">Income, spending & monthly balance</div></div>
 ...
 <div class="card hero"><div class="muted">Current savings</div><div class="big">${euro(currentSavings())}</div><div class="row small"><span>Starting ${euro(state.startingSavings)}</span><span>Change ${euro(currentSavings()-state.startingSavings)}</span></div></div>
 <div class="card"><div class="row"><b>Monthly allowance</b><b>${euro(state.flexible)}</b></div><div class="row small"><span>Spent this month</span><span>${euro(spent())}</span></div><div class="row small"><span>Remaining</span><span>${euro(remaining())}</span></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button onclick="openAdd('expense')">＋ Add spending</button><button class="secondary" onclick="openAdd('income')">＋ Add income / profit</button></div></div>
 ${extraPaycheckSummary()}
 <div class="card">${months().map(k=>{let open=state.openMonths[k],out=spent(k),inc=incomeTotal(k),limit=state.flexible+(k===monthKey()?state.carry:0),diff=limit-out,cls=diff>=0?"under":"over",label=diff>=0?"Under limit":"Over limit";return `<div class="month-head ${cls}" onclick="toggleMonth('${k}')"><div class="row"><span><b>${monthLabel(k)}</b> <span class="status">${label}</span></span><span><b>${euro(out)}</b> ${open?"▲":"▼"}</span></div><div class="row small"><span>Income ${euro(inc)}</span><span>${diff>=0?euro(diff)+" left":euro(Math.abs(diff))+" over"}</span></div></div>${open?`<div>${txs(k).length?txs(k).map(x=>`<div class="item"><div class="row"><span>${esc(x.desc)} <span class="pill">${esc(x.cat)}</span></span><span class="${x.type==="income"?"cash-in":"spend-outflow"}">${x.type==="income"?"↑":"↓"} ${euro(x.amount)}</span></div><div style="text-align:right"><button class="icon delete trash-btn" onclick="event.stopPropagation();delTx('${x.id}')" class="trash-btn" aria-label="Delete">🗑<//button></div></div>`).join(""):'<div class="empty">No cash flow recorded.</div>'}</div>`:""}`}).join("")}</div>`;
}
...
function budgetDeductControls(key){
 ...
 return `<div class="budget-deduct-box">
   <div class="toggle"><div><b>Deduct on paycheck</b><div class="muted">Automatically deduct ${euro(info?.amount||0)} from savings when this month's paycheck is recorded.</div></div><div class="switch ${enabled?"on":""}" onclick="toggleBudgetDeduct('${esc(key)}')" role="switch" aria-checked="${enabled}"><i></i></div></div>
   <button class="secondary force-deduct-btn" onclick="forceBudgetDeduct('${esc(key)}')">↻ Force deduct this month</button>
 </div>`;
}
function budget(){
 const sections=[];
...
function openAdd(type="expense"){
  ...
  document.getElementById("sheet").innerHTML=`<h2>${income?"Add income / profit":"Add spending"}</h2><label>Description</label><input id="desc" placeholder="${income?"e.g. Side hustle":"e.g. Dinner"}"><label>Amount (€)</label><input id="amount" type="number" step=".01"><label>Category</label><select id="cat">${income?`<option>Income</option><option>Side hustle</option><option>Other</option>`:`<option>General</option><option>Food</option><option>Entertainment</option><option>Clothes</option><option>Transport</option><option>Gifts</option><option>Holiday</option><option>Other</option>`}</select><button onclick="addTx('${income?"income":"expense"}')">Add</button> <button class="secondary" onclick="closeModal()">Cancel</button>`;
  document.getElementById("modal").classList.add("open")
}
function addTx(type="expense"){
  ...
  state.transactions.push({id:Date.now().toString(),month:monthKey(),desc,amount,cat:document.getElementById("cat").value,type});
...
function toggleBudgetDeduct(key){
  ...
  state.budgetDeduct[key]={...(state.budgetDeduct[key]||{}),onPaycheck:!state.budgetDeduct[key]?.onPaycheck};
  save();render("budget");
}
...
function addCustomSection(){
  ...
  state.customSections.push({name:"New section",items:[],deductOnPaycheck:false});
